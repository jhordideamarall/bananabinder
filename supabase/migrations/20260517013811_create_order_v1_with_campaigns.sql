-- Extend create_order_v1 to apply campaigns server-side with full validation.
-- Anti-fraud: re-validate every campaign by ID (active, period, target, region,
-- min_order, usage_limit). Insert into campaign_usages atomically.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    INSERT INTO public.order_items (order_id, product_id, variant_id, product_name, variant_name, quantity, price, cost_price, discount, subtotal)
    VALUES (v_order_id, v_product_id, v_variant_id, v_product_name, v_variant_name, v_quantity, v_unit_price, v_cost_price, 0, v_line_subtotal);
