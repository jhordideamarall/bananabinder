...
import { IconTruck, IconCreditCard, IconChevronRight, IconTicket, IconMapPin, IconLoader2, IconCircleCheck } from "@tabler/icons-react";
          ...
          <section>
            ...
            {fetchingAddresses ? (
              <div className="flex items-center gap-2 text-gray-400 py-8">
                 <IconLoader2 className="w-5 h-5 animate-spin" /> Mencari alamat Anda...
              </div>
            ) : (
                    ...
                    <CardContent className="p-5 flex justify-between items-start">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                           ...
                           {addr.is_default && <Badge className="bg-primary/20 text-primary border-none text-[10px]">Utama</Badge>}
                        </div>
                        <p className="text-sm text-gray-500">{addr.phone}</p>
                        <p className="text-sm text-gray-600 line-clamp-2 max-w-md">{addr.full_address}</p>
                      </div>
                      {selectedAddress?.id === addr.id && <IconCircleCheck className="w-6 h-6 text-primary" />}
                    </CardContent>
                  </Card>
                ))}
                   ...
                   >
                     <CardContent className="p-4 flex justify-between items-center">
                        <div className="text-left">
                           <p className="font-black uppercase text-xs tracking-widest text-primary">{r.company}</p>
                           <p className="font-bold text-sm text-gray-900">{r.service}</p>
                           <p className="text-[10px] text-gray-400 font-bold uppercase">{r.duration}</p>
                        </div>
          ...
          <Card className="glass sticky top-24 border-white/40 shadow-2xl overflow-hidden">
             ...
             <CardContent className="p-8">
                ...
                <div className="space-y-4 mb-8">
                  ...
                  <div className="flex justify-between text-gray-500 text-sm font-medium">
                    ...
                    {calculatingShipping ? (
                      ...
                      <span>Rp {(selectedRate?.price || 0).toLocaleString("id-ID")}</span>
                    )}
                  </div>
                  <div className="pt-6 border-t border-gray-100 flex justify-between items-center">
                    <span className="font-bold text-gray-900">Total Tagihan</span>
                    <span className="font-black text-2xl text-primary">
                      Rp {(totalPrice() + (selectedRate?.price || 0)).toLocaleString("id-ID")}
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-8 pt-8 border-t border-gray-50 flex justify-center gap-4 grayscale opacity-50">
                   <img src="https://upload.wikimedia.org/wikipedia/commons/e/eb/Logo_Xendit.png" alt="Xendit" className="h-4 object-contain" />
                </div>
             </CardContent>
          </Card>
        </div>
