...
export async function POST(req: Request) {
  try {
    ...
    if (!variant_id || !quantity) {
      return NextResponse.json({ error: "Varian dan jumlah wajib diisi" }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: (error as Error).message || "Gagal memperbarui keranjang" }, { status: 500 });
  }
}
