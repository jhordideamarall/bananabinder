...
export default async function OrdersHistoryPage() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "paid": return <IconCheck className="w-4 h-4" />;
      case "processing": return <IconPackage className="w-4 h-4" />;
      case "shipped": return <IconTruck className="w-4 h-4" />;
      case "delivered": return <IconCheck className="w-4 h-4" />;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="mb-10">
        ...
        <p className="text-gray-500 font-medium">Pantau status dan detail semua pembelianmu di sini.</p>
      </div>
        ...
        {orders.length === 0 ? (
          <Card className="border-none shadow-xl shadow-gray-100/50 text-center py-20">
            <CardContent>
              ...
              <h3 className="text-lg font-black text-gray-900">Belum ada pesanan</h3>
              <p className="text-gray-500 mt-2">Mulai belanja binder favoritmu sekarang!</p>
              <Link href="/">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-sm font-black text-gray-900 group-hover:text-primary transition-colors">#{order.id.slice(0, 8)}</span>
                          <Badge className={`${getStatusColor(order.status)} border-none text-[10px] font-black uppercase tracking-widest px-2 py-0.5`}>
                        ...
                        <p className="text-sm font-bold text-gray-600 mt-2">
                          {order.items.length} Produk • <span className="text-primary">Rp {order.total_amount.toLocaleString("id-ID")}</span>
                        </p>
                      </div>
                    </div>
